<template>
    <div class="chat-box" :hidden="!chatVisible" v-bind:style="{ height: chatBoxHeight + 'vw' }">
        <div class="chat-list" ref="chatList"
             ...
             v-bind:class="{ overflow_visible: inputActive, overflow_hidden: !inputActive }">
            ...
            <div v-for="(message, index) in messages" v-bind:key="index">
                <div v-bind:style="{ 'background-color': message.backgroundColor }" class="message-box">
                    ...
                    }">{{ getTimestamp(message.sendetAt) }} {{ message.beforeChat }}<span v-bind:style="{ color: message.nameColor }">{{ message.sender }}</span>{{ message.afterName }}<span v-html="message.context"></span>{{ message.afterChat }}</span>
                </div>
            </div>
        </div>
...
export default class Chat extends Vue {
    ...
    private handleSuggestions(): void {
        ...
        this.cachedCommands.forEach((command: CommandInterface) => {
            if (command.name.toLowerCase().includes(message[0].toLowerCase())
                || command.aliases.find(n => n.toLowerCase().includes(message[0].toLowerCase()))) {
        ...
        suggestions = suggestions.sort((a, b) => {
            if (a.name.toLowerCase() == message[0].toLowerCase())
                return -1;
    ...
    private getTimestamp(timeJson: string): string {
        ...
        return "[" + date.toLocaleTimeString("de-DE", {hour: 'numeric', minute: 'numeric', second: 'numeric'}) + "]";
    }
...
.chat-message {
    font-weight: 600;
    font-size: 0.9vw;
    text-shadow: -0.05vw -0.05vw 0 rgb(0, 0, 0),
    0.05vw -0.05vw 0 rgb(0, 0, 0),
    -0.05vw 0.05vw 0 rgb(0, 0, 0),
    0.05vw 0.05vw 0 rgb(0, 0, 0);
}
