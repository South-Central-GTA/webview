<template>
    <div class="character-menu-groups">
        ...
        <div :hidden="groupSelected">
            <div class="groups-block">
                <div v-for="(group) in groups" v-bind:key="group.id">
                    <button type="button" class="group-button btn btn-primary w-100"
                            @click="selectGroup(group)">{{ group.name }} #{{ group.id }}
                    </button>
                </div>
        ...
        <div :hidden="!groupSelected">
            <leave-group-page ref="leaveGroupPage"
                              :hidden="pageIndex !== 0"
                              v-on:back="resetPage()"
                              v-on:leave="leaveGroup()"/>
            
            <group-ranks-settings-page ref="groupRanksSettingsPage"
                                       :hidden="pageIndex !== 1"
                                       v-on:back="resetPage()"/>
            
            <close-group-page ref="closeGroupPage"
                              :hidden="pageIndex !== 2"
                              v-on:back="resetPage()"
                ...
                <div class="pt-1">
                    ...
                    <div class="section-box mt-2" v-if="isOwner">
                        <div class="row mb-2">
                            ...
                            <div class="col-8">
                                <input type="text"
                                       @input="search()" v-model="groupMemberSearch"
                                       id="search-bar"
...
export default class CharacterMenuGroups extends Vue {
    ...
    public mounted(): void {
        group.getInstance().AllGroupsChanged.on((groups: GroupInterface[] | undefined) => this.setup(groups));
    ...
    private selectGroup(group: GroupInterface): void {
        ...
        if (company !== undefined) {
            this.hasVehicleDealer = (company.licenses & LicensesType.VEHICLE_DEALERSHIP) === LicensesType.VEHICLE_DEALERSHIP;
        }
    ...
    private search(): void {
        ...
        this.groupMembers = this.cachedGroupMembers.filter(gm => gm.characterName.toLowerCase().includes(this.groupMemberSearch.toLowerCase()));
    }
}
</script>
