<template>
    <div class="close-group-page">
        ...
        <p class="card-text">Hier kannst du die Gruppe löschen,
            bedenke jedoch das du dies dennoch ausspielen musst. Einige Gruppierungen kann man IC nicht so einfach
            löschen.</p>

        <p class="card-text">Bedenke das die Gruppe <strong>sofort</strong> gelöscht wird. Der Hauptsitz und
            pachtbare Unternehmenssitze werden verkauft,
            Lizenzen sowie der Name gelöscht. Alle IC Mitglieder sofort rausgeschmissen und jegliche
            Gruppenspezifische Information gelöscht.</p>

        <input type="text" class="form-control" ref="securityQuestionInput"
               @input="validateButton()"
               @focus="onFocus(true)"
               @blur="onFocus(false)"
               :placeholder="'Gebe den Name der Gruppe an. (' + groupName + ')'">

        <button type="button" class="btn btn-danger w-100 mt-3" :disabled="!closeButtonEnabled"
                @click="deleteGroup()">
